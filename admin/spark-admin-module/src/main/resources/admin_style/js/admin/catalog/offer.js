...
(function($, SCAdmin) {
    ...
    // Add utility functions for offers to the SCAdmin object
    SCAdmin.offer = {
    
        addOnChangeTriggers : function($form) {
            $form.find('#field-type').on('change', function() {
                SCAdmin.offer.initializeOfferTypeField($form);
            });
            
            $form.find('#field-deliveryType').on('change', function() {
                SCAdmin.offer.initializeDeliveryTypeField($form);
            });
        },
    ...
    SCAdmin.addInitializationHandler(function($container) {
        ...
        SCAdmin.offer.addOnChangeTriggers($form);
        SCAdmin.offer.initializeOfferFormFields($form);
    });
