...
$(document).ready(function() {
	...
	$('body').on('submit', 'form.entity-form', function(event) {
        var submit = SCAdmin.runSubmitHandlers($(this));
        return submit;
	});
	
    $('body').on('click', 'button.add-main-entity', function(event) {
    	SCAdmin.showLinkAsModal($(this).data('url'));
    	return false;
    });
    
    $('body').on('click', 'a.add-main-entity-select-type', function(event) {
    	SCAdmin.modalNavigateTo($(this).attr('href'));
    	return false;
    });
        ...
        if (submit) {
    		SC.ajax({
    			...
    			if (errorDiv.length) {
    			    ...
    			    var currentErrorDiv = SCAdmin.currentModal().find('.modal-body .errors')
    			    if (currentErrorDiv.length) {
    			        currentErrorDiv.replaceWith(errorDiv)
    			    } else {
    			        SCAdmin.currentModal().find('.modal-body').prepend(errorDiv);
    			    }
    			}
    			SCAdmin.initializeFields($('.modal .modal-body .tabs-content'));
