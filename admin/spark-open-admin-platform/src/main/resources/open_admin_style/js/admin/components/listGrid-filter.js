...
(function($, SCAdmin) {
    ...
    SCAdmin.listGrid.filter = {
        ...
        clearActiveSorts : function($context) {
            ...
            SCAdmin.history.replaceUrlParameter('sortProperty', null);
            SCAdmin.history.replaceUrlParameter('sortDirection', null);
        },
        ...
        initialize : function(context) {
            ...
            var params = SCAdmin.history.getUrlParameters();
            if (!params) {
                        ...
                        if ($parent.hasClass('additional-foreign-key-container')) {
                            ...
                            if ($parent.find('.datepicker').length > 0) {
                                ...
                                if (rangeValue.indexOf('|') >= 0) {
                                    ...
                                    vals[0] = SCAdmin.dates.getDisplayDate(vals[0]);
                                    vals[1] = SCAdmin.dates.getDisplayDate(vals[1]);
                                    rangeValue = vals[0] + '|' + vals[1];
                                } else {
                                    rangeValue = SCAdmin.dates.getDisplayDate(rangeValue);
                                }
                            }
    ...
    $('.listgrid-headerBtn.dropdown .add-filter').click(function (e) {
        ...
        $(criteria).append(SCAdmin.listGrid.filter.createCriteria);
        SCAdmin.listGrid.filter.showOperators(criteria);
    });
    ...
    $(document).on('click.fndtn', '.listgrid-headerBtn.dropdown:not(.split), .listgrid-headerBtn.dropdown.split i.filter-icon', function (e) {
        ...
        setTimeout(function () {
            ...
            SCAdmin.listGrid.filter.closeDropdowns(dropdown);
        }, 0);
    ...
    $(document).on('click.fndtn', 'body, html', function (e) {
        ...
        if (!$(e.originalEvent.target).parents().is('.listgrid-headerBtn') &&
            ...
            SCAdmin.listGrid.filter.closeDropdowns();
            if (dropdownAsToggle) {
                SCAdmin.listGrid.filter.resetToggles();
            }
        }
    });
...
$(document).ready(function() {
    ...
    $('body').on('click', '.icon-sort', function() {
        ...
        SCAdmin.listGrid.filter.clearActiveSorts($(this));
    ...
    $('body').on('click', 'button.listgrid-clear-filter', function(event) {
        ...
        if ($tbody.data('listgridtype') == 'main') {
            ...
            SCAdmin.history.replaceUrlParameter(name, null);
        }
    ...
    $('body').on('click', 'div.filter-fields button.listgrid-filter', function(event, indirect) {
        ...
        $(this).closest('thead').find('div.filter-fields input.datepicker').each(function(i, e) {
            var serverVal = SCAdmin.dates.getServerDate($(e).val());
        ...
        var $tbody = $(this).closest('.listgrid-container').find('.listgrid-body-wrapper .list-grid-table');
        SCAdmin.listGrid.showLoadingSpinner($tbody, $tbody.closest('.mCustomScrollBox').position().top + 3);
        SC.ajax({
            ...
            if ($tbody.data('listgridtype') == 'main') {
                ...
                $(nonBlankInputs).each(function(index, input) {
                    SCAdmin.history.replaceUrlParameter(input.name, input.value);
                });
            }
            SCAdmin.listGrid.hideLoadingSpinner($tbody);
            SCAdmin.listGrid.replaceRelatedListGrid($(data).find('div.listgrid-header-wrapper'), null, { isRefresh : false });
            $inputs.each(function(index, input) {
    ...
    $('body').on('click', '.custom-entity-search a.search-button', function(event) {
        ...
        SC.ajax({
            ...
            SCAdmin.history.replaceUrlParameter('startIndex');
            for (key in submitData) {
                SCAdmin.history.replaceUrlParameter(key, submitData[key]);
            }
            SCAdmin.listGrid.replaceRelatedListGrid($(data), null, { isRefresh : false});
