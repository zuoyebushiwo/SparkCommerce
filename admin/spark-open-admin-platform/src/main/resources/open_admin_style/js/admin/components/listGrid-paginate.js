...
(function($, SCAdmin) {
    ...
    SCAdmin.listGrid.paginate = {
        ...
        loadRecords : function($tbody, baseUrl) {
            ...
            if (startIndex != null && maxIndex != null) {
                ...
                SCAdmin.listGrid.showLoadingSpinner($tbody, spinnerOffset);
                
                var url = SCAdmin.history.getUrlWithParameter('startIndex', startIndex, null, baseUrl);
                url = SCAdmin.history.getUrlWithParameter('maxIndex', maxIndex, null, url);
                
                var params = $tbody.closest('.listgrid-container').find('.listgrid-header-wrapper table').data('currentparams');
                for (var param in params) {
                    url = SCAdmin.history.getUrlWithParameter(param, params[param], null, url);
                }
                
                //console.log('Loading more records -- ' + url);
                
                SC.ajax({ url: url, type: 'GET' }, function(data) {
                    var $newTbody = data.find('tbody');
                    SCAdmin.listGrid.paginate.injectRecords($tbody, $newTbody);
                    SCAdmin.listGrid.paginate.releaseLock();
        ...
        updateGridSize : function($tbody) {
            ...
            if ($table.data('listgridtype') == 'main') {
                ...
                if ($wrapper.find('.mCS_no_scrollbar').length > 0) {
                    SCAdmin.listGrid.paginate.updateUrlFromScroll($wrapper.find('tbody'));
                    SCAdmin.listGrid.paginate.updateTableFooter($wrapper.find('tbody'));
                }
        ...
        updateUrlFromScroll : function($tbody) {
            var topIndex = this.getTopVisibleIndex($tbody);
            if (topIndex > 0) {
                SCAdmin.history.replaceUrlParameter('startIndex', topIndex);
            } else {
        ...
        initialize : function($container) {
            ...
            SCAdmin.listGrid.paginate.updateGridSize($tbody);
            ...
            $wrapper.mCustomScrollbar({
                ...
                callbacks: {
                    onScroll: function() {
                        ...
                        SCAdmin.listGrid.paginate.updateTableFooter($tbody);
                        ...
                        $.doTimeout('fetch', fetchDebounce, function() {
                            ...
                            SCAdmin.listGrid.paginate.loadRecords($tbody, url);
                        });
                        
                        // Also update the URL if we're on a main list grid
                        if (isMainGrid) {
                            $.doTimeout('updateurl', updateUrlDebounce, function(){
                                SCAdmin.listGrid.paginate.updateUrlFromScroll($tbody);
                            });
                        }
                    }
                }
    ...
    SCAdmin.addUpdateHandler(function($container) {
        $container.find('.needsupdate').each(function(index, element) {
            SCAdmin.listGrid.paginate.updateGridSize($(element));
