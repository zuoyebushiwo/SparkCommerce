 ...
 * #L%
 */
/* Utility methods provided by Spark Commerce for admin */
var SCAdmin = (function($) {
	...
	function showModal($data, onModalHide, onModalHideArgs) {
		...
		$data.on('hidden', function() {
			...
			if (SCAdmin.currentModal()) {
				SCAdmin.currentModal().find('.submit-button').show();
				SCAdmin.currentModal().find('img.ajax-loader').hide();
			}
		});
		
		SCAdmin.initializeModalTabs($data);
        SCAdmin.initializeModalButtons($data);
		SCAdmin.setModalMaxHeight(SCAdmin.currentModal());
	...
	return {
        ...
        runSubmitHandlers : function($form) {
            SCAdmin.runPreValidationSubmitHandlers($form);
            var submit = SCAdmin.runValidationSubmitHandlers($form);
            SCAdmin.runPostValidationSubmitHandlers($form);
            return submit;
        },
    	...
    	showMessageAsModal : function(header, message) {
			if (SCAdmin.currentModal() != null && SCAdmin.currentModal().hasClass('loading-modal')) {
    	...
    	showLinkAsModal : function(link, onModalHide, onModalHideArgs) {
    	    ...
    	    $modal.find('.modal-header h3').text(SCAdmin.messages.loading);
    	...
    	modalNavigateTo : function(link) {
    		if (SCAdmin.currentModal()) {
    		    ...
    		    SC.ajax({
        			...
        			$data = $data.children();
        		    SCAdmin.currentModal().empty().append($data);
        			...
        			SCAdmin.currentModal().removeClass('loading-modal');
        		});
    		} else {
    	...
    	hideCurrentModal : function() {
    		if (SCAdmin.currentModal()) {
    			SCAdmin.currentModal().modal('hide');
    		}
    	},
    	
    	focusOnTopModal : function() {
    	    if (SCAdmin.currentModal()) {
    	        SCAdmin.currentModal().focus();
    	    }
    	},
...
SC.defaultErrorHandler = function(data) {
    if (data.status == "403") {
        SCAdmin.showMessageAsModal(SCAdmin.messages.error, SCAdmin.messages.forbidden403);
    } else {
        var $data;
        ...
        if ($data.length == 1) {
            SCAdmin.showElementAsModal($data);
        } else {
            // This shouldn't happen, but it's here as a fallback just in case
            SCAdmin.showMessageAsModal(SCAdmin.messages.error, SCAdmin.messages.errorOccurred);
        }
    }
}

SC.addPreAjaxCallbackHandler(function($data) {
...
$(window).on('scroll', function() {
    ...
    if ($(window).scrollTop() < SCAdmin.getOriginalStickyBarOffset()) {
