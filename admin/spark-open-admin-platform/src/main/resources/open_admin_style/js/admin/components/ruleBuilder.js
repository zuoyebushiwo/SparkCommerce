    ...
    SCAdmin.addInitializationHandler(function($container) {
        $container.find('.rule-builder-data').each(function(index, element) {
            var $this = $(this),
                ...
                data = $this.data('data'),
                condition = SCAdmin.conditions.addCondition(hiddenId, containerId, fields, data);
            
            SCAdmin.conditions.initializeCondition($this.parent(), condition);
        });
        ...
        $container.find('.rule-builder-required-field').each(function(index, element) {
            ...
            SCAdmin.conditions.showOrCreateMainCondition($container, ruleType);
        });
    });
    
    SCAdmin.addPostValidationSubmitHandler(function($form) {
        for (var i = 0; i < SCAdmin.conditions.conditionCount(); i++) {
            var condition = SCAdmin.conditions.getConditionByIndex(i);
...
$(document).ready(function() {
    ...
    $('body').on('click', 'a.add-main-condition', function() {
        ...
        SCAdmin.conditions.addAdditionalMainCondition($container);
        return false;
    });
    ...
    $('body').on('change', 'input.clear-rules', function(){
        ...
        SCAdmin.conditions.hideMainCondition($container, 'add-main-rule');
    });
    ...
    $('body').on('click', 'a.rule-reset', function(){
        ...
        SCAdmin.conditions.getCondition($ruleData.data('containerid')).data.error = '';
        return false;
    });
