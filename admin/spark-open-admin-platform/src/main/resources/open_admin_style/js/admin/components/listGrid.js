...
(function($, SCAdmin) {
    ...
    // Add utility functions for list grids to the SCAdmin object
    SCAdmin.listGrid = {
        replaceRelatedListGrid : function($headerWrapper, alert, opts) {
            ...
            if (oldParams != undefined && oldParams != newParams && (opts == undefined || opts.isRefresh)) {
                ...
                for (var param in oldParams) {
                    url = SCAdmin.history.getUrlWithParameter(param, oldParams[param], null, url);
                }
            ...
            $.when(secondaryFetch.promise()).then(function(status) {
                var currentIndex = SCAdmin.listGrid.paginate.getTopVisibleIndex($oldTable.find('tbody'));
                ...
                SCAdmin.listGrid.initialize($listGridContainer);
                
                SCAdmin.listGrid.paginate.scrollToIndex($listGridContainer.find('tbody'), currentIndex);
                $listGridContainer.find('.listgrid-body-wrapper').mCustomScrollbar('update');
                
                SCAdmin.listGrid.paginate.updateTableFooter($listGridContainer.find('tbody'));
                
                if (alert) {
                    SCAdmin.listGrid.showAlert($listGridContainer, alert.message, alert);
                }
        ...
        initialize : function($container) {
            SCAdmin.listGrid.updateToolbarRowActionButtons($container);
            
            if (SCAdmin.listGrid.paginate) {
                SCAdmin.listGrid.paginate.initialize($container);
            }
            
            if (SCAdmin.listGrid.filter) {
                SCAdmin.listGrid.filter.initialize($container);
            }
        }
    };
    
    SCAdmin.addInitializationHandler(function($container) {
        $container.find('.listgrid-container').each(function(index, element) {
            SCAdmin.listGrid.initialize($(element));
        });
    });
...
$(document).ready(function() {
    ...
    $('body').on('click', '.list-grid-table tbody tr', function() {
        ...
        var fields = SCAdmin.listGrid.getRowFields($tr);
    ...
    function inlineRowSelected(event, link, fields, currentUrl, multi) {
        ...
        SCAdmin.listGrid.updateToolbarRowActionButtons($listGridContainer);
    }
    ...
    $('body').on('listGrid-basic-rowSelected', function(event, link, fields, currentUrl) {
        ...
        SC.ajax({
            ...
            SCAdmin.listGrid.replaceRelatedListGrid($(data), { 
                message: SCAdmin.messages.saved + '!', 
    ...
    $('body').on('listGrid-adorned-rowSelected', function(event, link, fields, currentUrl) {
        ...
        var $modal = SCAdmin.currentModal();
    ...
    $('body').on('click', '.to-one-lookup', function(event) {
            ...
            if (onChangeTrigger) {
                ...
                if (trigger[0] == 'dynamicForm') {
                    ...
                    SC.ajax({
                        ...
                        SCAdmin.initializeFields($newDynamicContainer);
        ...
        SCAdmin.showLinkAsModal($(this).data('select-url'), function() {
    ...
    $('body').on('click', 'button.sub-list-grid-add', function() {
        SCAdmin.showLinkAsModal($(this).attr('data-actionurl'));
        return false;
    });
    ...
    $('body').on('click', 'button.sub-list-grid-reorder', function() {
        ...
        if (doneReordering) {
            ...
            $(this).append(' ' + SCAdmin.messages.reorder);
            ...
            $(this).removeAttr('disabled').html(SCAdmin.messages.done);
            
            $trs.removeClass('clickable').addClass('draggable');
            
            $tbody.sortable({
                helper : SCAdmin.listGrid.fixHelper,
                update : function(event, ui) {
                    SC.ajax({
                        ...
                        SCAdmin.listGrid.showAlert($container, SCAdmin.messages.saved + '!', { 
                            alertType: 'save-alert', 
                            autoClose: 400 
                        });
                    });
    ...
    $('body').on('click', 'button.sub-list-grid-remove', function() {
        var link = SCAdmin.listGrid.getButtonLink($(this));
    ...
    $('body').on('submit', 'form.modal-form', function(event) { 
        var $form = $(this);
        var submit = SCAdmin.runSubmitHandlers($form);
        ...
        if (submit) {
            SC.ajax({
                ...
                if ($newForm[0]) {
                    ...
                    //since we just replaced everything, initialize all the fields back. See SCAdmin.showModal
                    SCAdmin.initializeModalTabs($(SCAdmin.currentModal()));
                    SCAdmin.initializeModalButtons($(SCAdmin.currentModal()));
                    SCAdmin.initializeFields();

            	    var $actions = SCAdmin.currentModal().find('.entity-form-actions');
