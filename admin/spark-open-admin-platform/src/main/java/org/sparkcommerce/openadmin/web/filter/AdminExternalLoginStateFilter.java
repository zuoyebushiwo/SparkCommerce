 ...
 * another system it is important to provision an admin user in Spark and set up the appropriate roles.
...
public class AdminExternalLoginStateFilter extends GenericFilterBean {
    ...
    protected static final String SC_ADMIN_PROVISION_USER_CHECK = "SC_ADMIN_PROVISION_USER_CHECK";
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (request.getSession(true).getAttribute(SC_ADMIN_PROVISION_USER_CHECK) == null) {
            ...
            if (authentication != null && authentication.isAuthenticated()) {
                if (authentication.getPrincipal() instanceof UserDetails){
                    ...
                    if (userDetails != null && userDetails.getUsername() != null) {
                        ...
                        if (userDetails instanceof SparkExternalAuthenticationUserDetails) {
                            SparkExternalAuthenticationUserDetails sparkUser = (SparkExternalAuthenticationUserDetails)userDetails;
                            if (user == null) {
                                //Provision a new user...
                                user = (AdminUser)entityConfiguration.createEntityInstance(AdminUser.class.getName());
                            }
                            saveAdminUser(sparkUser, user);
                            request.getSession().setAttribute(SC_ADMIN_PROVISION_USER_CHECK, Boolean.TRUE);
                        }
    ...
    protected void saveAdminUser(SparkExternalAuthenticationUserDetails sparkUser, AdminUser user) {
        //Name, login, password, email are required.
        user.setLogin(sparkUser.getUsername());
        user.setUnencodedPassword(sparkUser.getPassword());
        ...
        if (sparkUser.getFirstName() != null && sparkUser.getFirstName().trim().length() > 0) {
            name.append(sparkUser.getFirstName().trim());
            name.append(" ");
        }

        if (sparkUser.getLastName() != null && sparkUser.getLastName().trim().length() > 0) {
            name.append(sparkUser.getLastName().trim());
        }

        user.setName(name.toString());
        user.setEmail(sparkUser.getEmail());
        ...
        if (availableRoles != null) {
            ...
            Collection<GrantedAuthority> authorities = sparkUser.getAuthorities();
