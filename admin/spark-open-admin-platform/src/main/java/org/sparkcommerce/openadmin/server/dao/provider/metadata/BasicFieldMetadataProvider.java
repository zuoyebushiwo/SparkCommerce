...
import org.sparkcommerce.common.enumeration.domain.DataDrivenEnumerationValueImpl;
...
import org.sparkcommerce.common.presentation.override.AdminPresentationDataDrivenEnumerationOverride;
...
import org.sparkcommerce.common.presentation.override.AdminPresentationToOneLookupOverride;
...
import org.sparkcommerce.openadmin.server.dao.provider.metadata.request.AddMetadataRequest;
import org.sparkcommerce.openadmin.server.dao.provider.metadata.request.OverrideViaAnnotationRequest;
import org.sparkcommerce.openadmin.server.dao.provider.metadata.request.OverrideViaXmlRequest;
...
public class BasicFieldMetadataProvider extends FieldMetadataProviderAdapter {
    ...
    protected FieldMetadataOverride overrideMergeMetadata(AdminPresentationMergeOverride merge) {
        ...
        for (Map.Entry<String, AdminPresentationMergeEntry> entry : overrideValues.entrySet()) {
            ...
            if (entry.getKey().equals(PropertyType.AdminPresentation.FRIENDLYNAME)) {
                ...
                fieldMetadataOverride.setSparkEnumeration(stringValue);
    ...
    protected FieldMetadataOverride constructBasicMetadataOverride(AdminPresentation annot, AdminPresentationToOneLookup toOneLookup,
        ...
        if (annot != null) {
            ...
            override.setSparkEnumeration(annot.sparkEnumeration());
    ...
    protected void buildBasicMetadata(Class<?> parentClass, Class<?> targetClass, Map<String, FieldMetadata> attributes,
        ...
        if (basicFieldMetadata.getSparkEnumeration()!=null) {
            metadata.setSparkEnumeration(basicFieldMetadata.getSparkEnumeration());
        }
        if (!StringUtils.isEmpty(metadata.getSparkEnumeration()) && metadata.getFieldType()==SupportedFieldType.BROADLEAF_ENUMERATION) {
            try {
                setupSparkEnumeration(metadata.getSparkEnumeration(), metadata, dynamicEntityDao);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
