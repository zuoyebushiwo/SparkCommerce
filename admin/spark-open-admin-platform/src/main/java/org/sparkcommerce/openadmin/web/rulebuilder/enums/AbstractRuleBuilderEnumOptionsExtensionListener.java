 ...
 * #L%
 */
package org.sparkcommerce.openadmin.web.rulebuilder.enums;
...
public abstract class AbstractRuleBuilderEnumOptionsExtensionListener implements RuleBuilderEnumOptionsExtensionListener {
    ...
    public String getOptionValues() {
        ...
        for (Entry<String, Class<? extends SparkEnumerationType>> entry : getValuesToGenerate().entrySet()) {
            try {
                sb.append("var ").append(entry.getKey()).append(" = [");
                
                int i = 0;
                Map<String, ? extends SparkEnumerationType> types = getTypes(entry.getValue());
                for (Entry<String, ? extends SparkEnumerationType> entry2 : types.entrySet()) {
     ...
     */
    protected abstract Map<String, Class<? extends SparkEnumerationType>> getValuesToGenerate();
