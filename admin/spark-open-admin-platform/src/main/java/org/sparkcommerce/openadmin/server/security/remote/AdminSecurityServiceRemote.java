...
import org.sparkcommerce.openadmin.server.security.domain.AdminPermission;
...
import org.sparkcommerce.openadmin.server.service.persistence.validation.GlobalValidationResult;
...
public class AdminSecurityServiceRemote implements AdminSecurityService, SecurityVerifier {
    ...
    protected org.sparkcommerce.openadmin.server.security.service.AdminSecurityService securityService;
    ...
    @Override
    public org.sparkcommerce.openadmin.server.security.remote.AdminUser getAdminUser() throws ServiceException {
        AdminUser persistentAdminUser = getPersistentAdminUser();
        if (persistentAdminUser != null) {
            org.sparkcommerce.openadmin.server.security.remote.AdminUser response = new org.sparkcommerce.openadmin.server.security.remote.AdminUser();
            ...
            if (!securityService.doesOperationExistForCeilingEntity(permissionType, ceilingNames[0])) {
                if (LOG.isWarnEnabled()) {
                    LOG.warn("Detected security request for an unregistered ceiling entity (" + ceilingNames[0] + "). " +
                        ...
                        "SC_ADMIN_PERMISSION_ENTITY table. Note, depending on how the entity in question is used, you " +
                        "may need to add to SC_ADMIN_PERMISSION, SC_ADMIN_ROLE_PERMISSION_XREF and SC_ADMIN_SEC_PERM_XREF.", primaryException);
                }
            }
