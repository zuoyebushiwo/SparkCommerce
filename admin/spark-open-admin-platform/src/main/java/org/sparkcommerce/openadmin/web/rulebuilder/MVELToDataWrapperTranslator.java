...
import org.sparkcommerce.openadmin.web.rulebuilder.grouping.Group;
import org.sparkcommerce.openadmin.web.rulebuilder.grouping.GroupingTranslator;
import org.sparkcommerce.openadmin.web.rulebuilder.service.RuleBuilderFieldService;
import org.sparkcommerce.openadmin.web.rulebuilder.statement.Expression;
import org.sparkcommerce.openadmin.web.rulebuilder.statement.PhraseTranslator;
...
public class MVELToDataWrapperTranslator {
    ...
    protected DataDTO createRuleDataDTO(DataDTO parentDTO, Group group, RuleBuilderFieldService fieldService)
        ...
        if (group.getOperatorType() == null) {
            group.setOperatorType(SCOperator.AND);
        }
        SCOperator operator = group.getOperatorType();
        data.setGroupOperator(operator.name());
        List<ExpressionDTO> myCriteriaList = new ArrayList<ExpressionDTO>();
        if (SCOperator.NOT.equals(group.getOperatorType()) && group.getIsTopGroup()) {
    ...
    protected void postProcessCriteria(DataDTO parentDTO, List<ExpressionDTO> myCriteriaList,
        ...
        if (
            count > 0 &&
                ...
                myCriteriaList.get(count - 1).getOperator().equals(SCOperator.GREATER_THAN.name()) &&
                temp.getOperator().equals(SCOperator.LESS_THAN.name())
            ) {
            myCriteriaList.get(count-1).setOperator(SCOperator.BETWEEN.name());
            String start;
            String end;
            ...
            count > 0 &&
                temp.getName().equals(myCriteriaList.get(count-1).getName()) &&
                myCriteriaList.get(count-1).getOperator().equals(SCOperator.GREATER_OR_EQUAL.name()) &&
                temp.getOperator().equals(SCOperator.LESS_OR_EQUAL.name())
            ) {
            myCriteriaList.get(count - 1).setOperator(SCOperator.BETWEEN_INCLUSIVE.name());
            String start;
            String end;
            if (type.toString().equals(SupportedFieldType.DATE.toString())) {
                start = myCriteriaList.get(count-1).getValue();
