...
import org.sparkcommerce.core.web.api.wrapper.ErrorMessageWrapper;
import org.sparkcommerce.core.web.api.wrapper.ErrorWrapper;
...
@Scope("singleton")
@Provider
public class SparkRestExceptionMapper implements ExceptionMapper<Throwable>, MessageSourceAware, ApplicationContextAware {

    private static final Log LOG = LogFactory.getLog(SparkRestExceptionMapper.class);

    protected String messageKeyPrefix = SparkWebServicesException.class.getName() + '.';
    ...
    public Response toResponse(Throwable t) {
        ...
        if (t instanceof SparkWebServicesException) {
            //If this is a SparkWebServicesException, then we will build the components of the response from that.
            SparkWebServicesException blcException = (SparkWebServicesException) t;
            if (t.getCause() != null) {
            ...
            if (blcException.getMessages() != null && !blcException.getMessages().isEmpty()) {
                ...
                ErrorMessageWrapper errorMessageWrapper = (ErrorMessageWrapper) context.getBean(ErrorMessageWrapper.class.getName());
                errorMessageWrapper.setMessageKey(resolveClientMessageKey(SparkWebServicesException.UNKNOWN_ERROR));
                errorMessageWrapper.setMessage(messageSource.getMessage(SparkWebServicesException.UNKNOWN_ERROR, null,
                        SparkWebServicesException.UNKNOWN_ERROR, locale));
                errorWrapper.getMessages().add(errorMessageWrapper);
            }
     ...
     * The default is "org.sparkcommerce.core.web.api.SparkWebServicesException.". So, if a message key contained 
     * in a SparkWebServicesException is org.sparkcommerce.core.web.api.SparkWebServicesException.unknownError, 
