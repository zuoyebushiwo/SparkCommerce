 ...
 * #L%
 */
package org.sparkcommerce.core.web;
...
public class SpringTemporaryRedirectOverrideFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        SparkResponseWrapper responseWrapper = new SparkResponseWrapper((HttpServletResponse) response);
        chain.doFilter(request, responseWrapper);
        if ( 
                response instanceof SparkResponseWrapper && 
                302 == ((SparkResponseWrapper) response).getStatus() &&
