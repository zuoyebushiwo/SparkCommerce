...
import org.sparkcommerce.common.payment.service.PaymentGatewayConfigurationServiceProvider;
...
import org.sparkcommerce.core.workflow.state.ActivityStateManagerImpl;
...
public class ValidateAndConfirmPaymentActivity extends BaseActivity<ProcessContext<CheckoutSeed>> {
                ...
                for (PaymentTransaction tx : payment.getTransactions()) {
                    if (PaymentTransactionType.UNCONFIRMED.equals(tx.getType())) {
                        ...
                        if (PaymentType.CREDIT_CARD.equals(payment.getType())) {
                            ...
                            // This can happen if you send the Credit Card directly to Spark or you use a Digital Wallet solution like MasterPass.
    ...
    protected String getGatewayExpirationDateFormat(){
        String format = SCSystemProperty.resolveSystemProperty("gateway.config.global.expDateFormat");
