...
import org.sparkcommerce.profile.core.dao.CustomerDao;
...
public class OrderDaoImpl implements OrderDao {
    ...
    public Order createNewCartForCustomer(Customer customer) {
        ...
        if (customer.getUsername() == null) {
            ...
            if (customerDao.readCustomerById(customer.getId()) != null) {
                throw new IllegalArgumentException("Attempting to save a customer with an id (" + customer.getId() + ") " +
                        ...
                        "Spark customers, but the batchStart setting has not been declared for id generation. In " +
        ...
        if (SparkRequestContext.getSparkRequestContext() != null) {
            order.setCurrency(SparkRequestContext.getSparkRequestContext().getSparkCurrency());
            order.setLocale(SparkRequestContext.getSparkRequestContext().getLocale());
        }
    ...
    @Override
    public Order create() {
        final Order order = ((Order) entityConfiguration.createEntityInstance("org.sparkcommerce.core.order.domain.Order"));
    ...
    public Order readNamedOrderForCustomer(final Customer customer, final String name) {
        ...
        if (SparkRequestContext.getSparkRequestContext() != null) {
            ...
            while (iter.hasNext()) {
                Locale locale = SparkRequestContext.getSparkRequestContext().getLocale();
