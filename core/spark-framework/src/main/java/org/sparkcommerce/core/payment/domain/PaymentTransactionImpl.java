...
@Table(name = "SC_ORDER_PAYMENT_TRANSACTION")
@SQLDelete(sql="UPDATE SC_ORDER_PAYMENT_TRANSACTION SET ARCHIVED = 'Y' WHERE PAYMENT_TRANSACTION_ID = ?")
...
public class PaymentTransactionImpl implements PaymentTransaction {
    ...
    @GenericGenerator(
        ...
        parameters = {
            ...
            @Parameter(name="entity_name", value="org.sparkcommerce.core.payment.domain.PaymentTransactionImpl")
        }
    )
    ...
    @AdminPresentation(friendlyName = "PaymentTransactionImpl_Type",
            ...
            sparkEnumeration = "org.sparkcommerce.common.payment.PaymentTransactionType",
    ...
    @CollectionTable(name="SC_TRANS_ADDITNL_FIELDS", joinColumns=@JoinColumn(name="PAYMENT_TRANSACTION_ID"))
    @BatchSize(size = 50)
    ...
    public Money getAmount() {
        return amount == null ? SparkCurrencyUtils.getMoney(BigDecimal.ZERO, getOrderPayment().getCurrency()) : SparkCurrencyUtils.getMoney(amount, getOrderPayment().getCurrency());
    }
