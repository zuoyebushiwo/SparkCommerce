...
@SQLDelete(sql="UPDATE SC_ORDER_PAYMENT SET ARCHIVED = 'Y' WHERE ORDER_PAYMENT_ID = ?")
@DirectCopyTransform({
...
public class OrderPaymentImpl implements OrderPayment, CurrencyCodeIdentifiable {
    ...
    @GenericGenerator(
        ...
        parameters = {
            ...
            @Parameter(name="entity_name", value="org.sparkcommerce.core.payment.domain.OrderPaymentImpl")
        }
    )
    ...
    @AdminPresentation(friendlyName = "OrderPaymentImpl_Payment_Type", order=3000, gridOrder = 3000, prominent=true,
            ...
            sparkEnumeration="org.sparkcommerce.common.payment.PaymentType")
    ...
    @AdminPresentation(friendlyName = "OrderPaymentImpl_Gateway_Type", order=1000, gridOrder = 1000, prominent=true,
            fieldType = SupportedFieldType.BROADLEAF_ENUMERATION,
            sparkEnumeration="org.sparkcommerce.common.payment.PaymentGatewayType")
    ...
    public Money getTransactionAmountForType(PaymentTransactionType type) {
        Money amount = SparkCurrencyUtils.getMoney(BigDecimal.ZERO, getOrder().getCurrency());
        for (PaymentTransaction tx : getTransactions()){
