...
import org.sparkcommerce.core.inventory.service.ContextualInventoryService;
import org.sparkcommerce.core.inventory.service.InventoryUnavailableException;
...
public class DecrementInventoryRollbackHandler implements RollbackHandler<CheckoutSeed>{
    ...
    public static final String ROLLBACK_SC_INVENTORY_DECREMENTED = "ROLLBACK_SC_INVENTORY_DECREMENTED";
    public static final String ROLLBACK_SC_INVENTORY_INCREMENTED = "ROLLBACK_SC_INVENTORY_INCREMENTED";
    public static final String ROLLBACK_SC_ORDER_ID = "ROLLBACK_SC_ORDER_ID";
    public static final String EXTENDED_ROLLBACK_STATE = "SC_EXTENDED_ROLLBACK_STATE";
    ...
    public void rollbackState(Activity<? extends ProcessContext<CheckoutSeed>> activity, ProcessContext<CheckoutSeed> processContext, Map<String, Object> stateConfiguration)
        ...
        if (shouldExecute(activity, processContext, stateConfiguration)) {
            ...
            if (stateConfiguration.get(ROLLBACK_SC_ORDER_ID) != null) {
                orderId = String.valueOf(stateConfiguration.get(ROLLBACK_SC_ORDER_ID));
            }
            
            @SuppressWarnings("unchecked")
            Map<Sku, Integer> inventoryToIncrement = (Map<Sku, Integer>) stateConfiguration.get(ROLLBACK_SC_INVENTORY_DECREMENTED);
            @SuppressWarnings("unchecked")
            Map<Sku, Integer> inventoryToDecrement = (Map<Sku, Integer>) stateConfiguration.get(ROLLBACK_SC_INVENTORY_INCREMENTED);
    ...
    protected boolean shouldExecute(Activity<? extends ProcessContext<CheckoutSeed>> activity, ProcessContext<CheckoutSeed> processContext, Map<String, Object> stateConfiguration) {
        return stateConfiguration != null && (
                stateConfiguration.get(ROLLBACK_SC_INVENTORY_DECREMENTED) != null ||
                stateConfiguration.get(ROLLBACK_SC_INVENTORY_INCREMENTED) != null ||
