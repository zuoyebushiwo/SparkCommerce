...
public class BandedShippingModule implements ShippingModule {
    ...
    private void calculateShipping(FulfillmentGroup fulfillmentGroup) {
        ...
        if (fulfillmentGroup.getFulfillmentGroupItems().size() == 0) {
            ...
            fulfillmentGroup.setShippingPrice(SparkCurrencyUtils.getMoney(BigDecimal.ZERO, fulfillmentGroup.getOrder().getCurrency()));
            fulfillmentGroup.setSaleShippingPrice(SparkCurrencyUtils.getMoney(BigDecimal.ZERO, fulfillmentGroup.getOrder().getCurrency()));
            fulfillmentGroup.setRetailShippingPrice(SparkCurrencyUtils.getMoney(BigDecimal.ZERO, fulfillmentGroup.getOrder().getCurrency()));
            return;
        }
        ...
        fulfillmentGroup.setShippingPrice(SparkCurrencyUtils.getMoney(shippingPrice, fulfillmentGroup.getOrder().getCurrency()));
