    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        if (detectSequenceGeneratorInconsistencies) {
            ...
            for (Object item : sessionFactory.getAllClassMetadata().values()) {
                ...
                if (genericAnnot != null && genericAnnot.strategy().equals("org.sparkcommerce.common.persistence.IdOverrideTableGenerator")) {
                    //This is a SC style ID generator
                            ...
                            if (maxEntityId > maxSequenceId) {
                                if (automaticallyCorrectInconsistencies) {
                                    ...
                                    String reason = "A data inconsistency has been detected between the " + tableName + " table and one or more entity tables for which it manages current max primary key values.\n" +
                                        "The inconsistency was detected between the managed class (" + mappedClass.getName() + ") and the identifier (" + segmentValue + ") in " + tableName + ". Spark\n" +
                                        ...
                                        "from 1.6 or below, please refer to http://docs.sparkcommerce.org/current/1.6-to-2.0-Migration.html for important information regarding migrating your SEQUENCE_GENERATOR table.";
                                    LOG.error("Spark Commerce failed to start", new RuntimeException(reason));
                                    System.exit(1);
                                }
                            }
                        }
