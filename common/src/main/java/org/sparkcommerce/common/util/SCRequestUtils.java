 ...
 * @author jdasari
 */
public class SCRequestUtils {
    
    private static String OK_TO_USE_SESSION = "blOkToUseSession";
    
    /**
     * Spark "Resolver" and "Filter" classes may need to know if they are allowed to utilize the session.
     * SC uses a pattern where we will store an attribute in the request indicating whether or not the 
    ...
    /**
     * Sets whether or not Spark can utilize the session in request processing.   Used by the REST API
