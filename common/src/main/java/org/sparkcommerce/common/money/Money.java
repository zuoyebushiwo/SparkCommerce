...
import org.sparkcommerce.common.money.util.CurrencyAdapter;
import org.sparkcommerce.common.util.xml.BigDecimalRoundingAdapter;
...
public class Money implements Serializable, Cloneable, Comparable<Money>, Externalizable {
    ...
    protected static String getCurrencyCode(SparkCurrency blCurrency) {
        if (blCurrency != null) {
    ...
    public Money(SparkCurrency blCurrency) {
        this(0, Currency.getInstance(getCurrencyCode(blCurrency)));
    }

    public Money(BigDecimal amount, SparkCurrency blCurrency) {
        this(amount, Currency.getInstance(getCurrencyCode(blCurrency)));
    }

    public Money(BigDecimal amount, SparkCurrency blCurrency, int scale) {
    ...
    public static Currency defaultCurrency() {
        ...
        if (brc != null && brc.getSparkCurrency() != null) {
            assert brc.getSparkCurrency().getCurrencyCode()!=null;
            return Currency.getInstance(brc.getSparkCurrency().getCurrencyCode());
        }
