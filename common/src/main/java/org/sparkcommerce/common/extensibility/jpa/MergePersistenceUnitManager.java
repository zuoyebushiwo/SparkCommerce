...
import org.sparkcommerce.common.extensibility.jpa.convert.EntityMarkerClassTransformer;
import org.sparkcommerce.common.extensibility.jpa.copy.NullClassTransformer;
...
public class MergePersistenceUnitManager extends DefaultPersistenceUnitManager {
    ...
    protected List<SparkClassTransformer> classTransformers = new ArrayList<SparkClassTransformer>();
    ...
    protected Set<SparkClassTransformer> mergedClassTransformers;
    ...
    public void preparePersistenceUnitInfos() {
        ...
        try {
            ...
            for (PersistenceUnitInfo pui : mergedPus.values()) {
                for (SparkClassTransformer transformer : classTransformers) {
                    try {
                        ...
                        if (refined instanceof IllegalStateException) {
                            LOG.warn("A SparkClassTransformer is configured for this persistence unit, but Spring " +
                                    "reported a problem (likely that a LoadTimeWeaver is not registered). As a result, " +
                                    "the Spark Commerce ClassTransformer ("+transformer.getClass().getName()+") is " +
    ...
    public List<SparkClassTransformer> getClassTransformers() {
        return classTransformers;
    }

    public void setClassTransformers(List<SparkClassTransformer> classTransformers) {
