 ...
 * @author Anand Dasari
 */
public class SparkThymeleafViewResolver extends ThymeleafViewResolver {
    private static final Log LOG = LogFactory.getLog(SparkThymeleafViewResolver.class);
    
    @Resource(name = "blSparkThymeleafViewResolverExtensionManager")
    protected SparkThymeleafViewResolverExtensionManager extensionManager;
     ...
     *   which is provided as part of SC.js
     *   
     ...
     * we are unable to call it from the SparkThymeleafViewResolver
     */
     ...
     * Spark specific methods match the viewName, it delegates to the parent 
    ...
    protected View createView(final String viewName, final Locale locale) throws Exception {
        ...
        if (viewName.startsWith(AJAX_REDIRECT_URL_PREFIX)) {
            LOG.trace("[THYMELEAF] View {" + viewName + "} is an ajax redirect, and will be handled directly by SparkThymeleafViewResolver");
    ...
    /**
     * Performs a Spark AJAX redirect. This is used in conjunction with SC.js to support
    ...
    protected View loadAjaxRedirectView(String redirectUrl, final Locale locale) throws Exception {
        if (isAjaxRequest()) {
            ...
            addStaticVariable(SparkControllerUtility.SC_REDIRECT_ATTRIBUTE, redirectUrl);
    ...
    protected boolean isAjaxRequest() {
        // First, let's try to get it from the SparkRequestContext
        HttpServletRequest request = null;
        if (SparkRequestContext.getSparkRequestContext() != null) {
            HttpServletRequest brcRequest = SparkRequestContext.getSparkRequestContext().getRequest();
            if (brcRequest != null) {
                request = brcRequest;
            }
        }
        ...
        if (request == null) {
            try {
                ...
                LOG.warn("Unable to cast to ServletRequestAttributes and the request in SparkRequestContext " + 
        ...
        return SparkControllerUtility.isAjaxRequest(request);
    }
